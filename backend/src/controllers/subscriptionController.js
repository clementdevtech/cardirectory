...
const activateSubscription = async (req, res) => {
  try {
    ...
    if (!user_id || !plan) {
      return res
        .status(400)
        .json({ success: false, error: "Missing user_id or plan" });
    }
    ...
    if (!dealerId) {
      const { data: newDealer, error: createDealerError } = await supabase
        ...
        .insert([
          {
            user_id,
            full_name: "Dealer User", // can be overridden from frontend
            email: "", // optional
            status: "active",
          },
        ])
    ...
    // 4️⃣ Promote user to dealer in users table
    ...
    // 5️⃣ Ensure user_roles table reflects dealer role
    ...
    return res.json({
      success: true,
      message: "Subscription activated and user promoted to dealer.",
    });
  } catch (error) {
